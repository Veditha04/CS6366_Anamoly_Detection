...
def train_epoch(model, train_loader, optimizer, criterion, device):
    ...
    for batch in pbar:
        # Train loader returns only images
...
def validate(model, test_loader, criterion, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            # Test loader returns (img, label, category, defect_type, path)
...
def main():
    ...
    # Create model
    model = BaselineAutoencoder(in_channels=3, base_channels=32).to(device)
    ...
    best_model_path = os.path.join(models_dir, "baseline_ae_best_enhanced.pth")
    final_model_path = os.path.join(models_dir, "baseline_ae_final.pth")
